...
class Alumnos : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        boton1.setOnClickListener {
            ...
            registro.put("numero_control", et1.text.toString())
        ...
        boton2.setOnClickListener {
            val admin = AdminSQLiteOpenHelper(this, "administracion", null, 1)
            val bd = admin.writableDatabase
            val fila = bd.rawQuery("select nombre,grado from alumnos where numero_control=${et1.text.toString()}", null)
            if (fila.moveToFirst()) {
        ...
        boton3.setOnClickListener {
            val admin = AdminSQLiteOpenHelper(this, "administracion", null, 1)
            val bd = admin.writableDatabase
            val fila = bd.rawQuery("select numero_control,grado from alumnos where nombre='${et2.text.toString()}'", null)
            if (fila.moveToFirst()) {
        ...
        boton4.setOnClickListener {
            val admin = AdminSQLiteOpenHelper(this, "administracion", null, 1)
            val bd = admin.writableDatabase
            val cant = bd.delete("alumnos", "numero_control=${et1.text.toString()}", null)
            bd.close()
            et1.setText("")
            et2.setText("")
            et3.setText("")
            if (cant == 1)
                Toast.makeText(this, "Se borró el alumno con dicho número de control", Toast.LENGTH_SHORT).show()
            else
                Toast.makeText(this, "No existe un alumno con dicho número de control", Toast.LENGTH_SHORT).show()
        }
        boton5.setOnClickListener {
            ...
            registro.put("nombre", et2.text.toString())
            registro.put("grado", et3.text.toString())
            val cant = bd.update("alumnos", registro, "numero_control=${et1.text.toString()}", null)
            bd.close()
            if (cant == 1)
                Toast.makeText(this, "Se modificaron los datos del alumno", Toast.LENGTH_SHORT).show()
            else
                Toast.makeText(this, "No existe un alumno con el número de control ingresado", Toast.LENGTH_SHORT).show()
        }
